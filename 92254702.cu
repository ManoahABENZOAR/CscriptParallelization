...
__global__ void gpu_kernel(double *d_f, int len)
{
   // write your code here
	__shared__ double d_ff;
	...
	printf("%d \n",z);
	   for(int st = 0; st < NSTEP; st++) {
		...
		//for(i = 1; i < N-1; i++) {
       		for(j = 1; j < N-1; j++) {
		d_ff=d_f[i*512+j]+KAPPA*(d_f[i*512+j-1]+d_f[i*512+j+1]+d_f[(i-1)*512+j]+d_f[(i+1)*512+j]-4.0*d_f[i*512+j]);
			
			d_f[(512*512)+i*512+j] = d_ff;
			}
		//}
		
		//for(i = 0; i < N; i++)
       		for(j = 0; j < N; j++){
				d_ff= d_f[(512*512)+i*512+j];
				d_f[i*512+j] = d_ff;
			}
...
{ 
   ...
   // gpu_kernel<<<grid, block>>>(...);
		gpu_kernel<<<grid, block>>>(d_f, 2*N*N);
